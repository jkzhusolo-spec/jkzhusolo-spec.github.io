...
The site is a single-page application with six navigation sections, served as a pure static site via GitHub Pages.

网站为单页应用，通过 GitHub Pages 静态托管，包含六个导航模块。
...
This portfolio site is intentionally lightweight — no frameworks, no build steps, no dependencies.

本作品集网站刻意保持轻量——无框架、无构建流程、无外部依赖。
...
- **No build tools** — edit and push, that's it · 无需构建工具，修改即部署
